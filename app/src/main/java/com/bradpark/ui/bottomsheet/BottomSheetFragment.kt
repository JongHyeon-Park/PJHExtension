...
import com.bradpark.calllback.BottomSheetStateCallback
 ...
 */
class BottomSheetFragment(private val mBottomSheetType: PjhConfig.PjhBottomSheetType, private val mBottomSheetButtonType: PjhConfig.PjhBottomSheetButtonType, private val title: String, private val mContentViewResId: Int, private val mButtonViewResId: Int = 0, private val mHasFullHeight: Boolean, private val cancelable: Boolean = true, private val calendarType: CalendarPagerAdapter.CalendarType = CalendarPagerAdapter.CalendarType.Normal,
                          private val mCalendarColor: CalendarColor? = null, private val mIsCalendarHeightFixed: Boolean = false, val selectDay: Calendar? = null, val inActiveDay: Calendar? = null, private val mBottomSheetStateCallback: BottomSheetStateCallback? = null, private val isMaxHeightFix : Boolean = false): BottomSheetDialogFragment() {
    companion object {
...
//            binding.clBottomSheetParent.maxHeight = calcHeight
    ...
    fun maxHeightSetting() {
        ...
        layout?.apply {
            ...
            val calcHeight = (displayMetrics.heightPixels -  PjhUi.convertDpToPixels(context!!, 64f)).toInt()
            layout.updateLayoutParams { height = WRAP_CONTENT }
    ...
    private fun initView() {
        ...
        if (mBottomSheetButtonType != PjhConfig.PjhBottomSheetButtonType.None) {
            ...
            (buttonView as BottomSheetButtonView).setButtonType(mBottomSheetButtonType)
            (buttonView as BottomSheetButtonView).setButtonStyle()
            (buttonView as BottomSheetButtonView).showView(activity = mActivity)
        }
    }
     ...
     */
    fun show(activity: AppCompatActivity): BottomSheetFragment? {
        mActivity = activity
